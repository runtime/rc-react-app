...
const Calendar = () => {
    ...
    const al = 4 //data.estimates[0].servicedetails.data.totalhours)
    ...
    return (
        <div className="Calendar">
            <Cal
                ...
                config={{
                    name: data.users[0].userdetails.firstname + " " + data.users[0].userdetails.lastname,
                    email: data.users[0].userdetails.email,
                    phone: data.users[0].userdetails.phone,
                    location: JSON.stringify({
                        value: "attendeeInPerson",
                        // It can be any string that defines an address where the meeting would occur
                        optionValue: data.locations[0].locationdetails.streetaddress + " " + data.locations[0].locationdetails.city + " " + data.locations[0].locationdetails.state + " " + data.locations[0].locationdetails.zip,
                    })
                }}
            />
        </div>
